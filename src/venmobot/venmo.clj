...
(defn render-oauth-link
  ...
  {:cookies {"slack_username" {:value username} "slack_token" {:value token}}
   :body (format "Hey, you need to <https://api.venmo.com/v1/oauth/authorize?client_id=%s&scope=make_payments&response_type=code|authenticate> before you can send money!" (System/getenv "VENMO_CLIENT_ID"))})
